...
def visualize(image, results, box_color=(0, 255, 0), text_color=(0, 0, 255), fps=None):
    ...
    if fps is not None:
        cv.putText(output, 'FPS: {:.2f}'.format(fps), (500, 15), cv.FONT_HERSHEY_SIMPLEX, 0.5, text_color)
    k = 0
    for det in (results if results is not None else []):
        ...
        y2 = bbox[1]+bbox[3] + 60
        print(x1, x2, y1,y2)
        # face = output[bbox[0]:bbox[1], bbox[0]+bbox[2]:bbox[1]+bbox[3]]
        face = output[x2:y2, x1:y1]
        ...
        print('eeee', fr.face_encodings(face, face_locations))
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            cv.putText(output, name, (bbox[0]+20, bbox[1]+12), cv.FONT_HERSHEY_DUPLEX, 0.5, text_color)
            cv.putText(output, name, (200, 200),cv.FONT_HERSHEY_DUPLEX, 0.5, text_color )
            print(name)
        # print('11')
        print(k)
    # return face
...
frame = visualize(frame, results, 122)
