...
while cv.waitKey(1) < 0:
    ...
    for det in (results if results is not None else []):
        bbox = det[0:4].astype(np.int32)
        cv.rectangle(frame, (bbox[0], bbox[1]), (bbox[0]+bbox[2], bbox[1]+bbox[3]), box_color, 2)
        # cv.rectangle(frame, (bbox[0]-40, bbox[1]-40), (bbox[0]+bbox[2]+40, bbox[1]+bbox[3]+40), box_color, 2)
        # conf = det[-1]
        # cv.putText(frame, '{:.4f}'.format(conf), (bbox[0], bbox[1]+12), cv.FONT_HERSHEY_DUPLEX, 0.5, text_color)
        ...
        if loop < 2:
            ...
            for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                ...
                if matches[best_match]:
                    name = known_names[best_match]
                name1.append(name)
                pos.append([bbox[0], bbox[1], bbox[0]+bbox[2], bbox[1]+bbox[3]])
                # cv.putText(frame, name, (bbox[0], bbox[1]+30), cv.FONT_HERSHEY_DUPLEX, 0.5, text_color)
                # # frame = frame.copy()
                # cv.putText(frame, name, (200, 200),cv.FONT_HERSHEY_DUPLEX, 0.5, text_color )
    ...
    for i in range(len(name1)):
        ...
        cv2.putText(frame, name1[i], (pos[i][0]+6, pos[i][1]-6), font, 1.0, (0, 0, 255), 1)
    loop += 1
    if loop >7:
        loop = 0
